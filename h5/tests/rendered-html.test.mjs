...
test("keeps preview controls on one page and aligns pointer coordinates with rendering", async () => {
  ...
  assert.match(appSource, /className="library-footer-actions">\s*<MiniProgramEntry \/>\s*<FeedbackContact \/>/);
  assert.match(appSource, /<WechatLogo size=\{16\} weight="bold" \/>\s*<span>小程序<\/span>/);
  assert.doesNotMatch(appSource, /DeviceMobile|使用小程序版/);
  ...
  assert.match(styleSource, /\.library-footer-actions\s*\{[\s\S]{0,240}display:\s*flex;[\s\S]{0,120}gap:\s*8px;[\s\S]{0,80}flex-wrap:\s*nowrap;/);
  assert.match(styleSource, /\.author-contact\s*\{[\s\S]{0,260}font-size:\s*12px;[\s\S]{0,80}font-weight:\s*650;/);
  assert.match(styleSource, /\.mini-program-trigger\s*\{[\s\S]{0,260}gap:\s*6px;[\s\S]{0,120}font-size:\s*12px;/);
  assert.match(styleSource, /\.feedback-notice\s*\{[\s\S]{0,180}position:\s*absolute;[\s\S]{0,180}white-space:\s*nowrap;/);
  ...
  assert.match(appSource, /stageResizeMenuDismissTimerRef/);
  assert.match(appSource, /window\.setTimeout\([\s\S]{0,500}, 3000\)/);
  assert.match(appSource, /toggleStageFit[\s\S]{0,180}fitRef\.current === "contain"/);
  assert.match(appSource, /双击切换完整或铺满/);
  assert.match(appSource, /className="stage-resizer-modes"/);
  assert.match(appSource, /aria-hidden=\{!stageResizeMenuActive\}/);
  assert.match(styleSource, /--spring-gentle:\s*cubic-bezier\(\.22, 1\.22, \.36, 1\)/);
  assert.match(styleSource, /\.stage-resizer-mode\s*\{[\s\S]{0,420}pointer-events:\s*none/);
  assert.match(styleSource, /\.stage-resizer\.is-selecting \.stage-resizer-mode\s*\{[\s\S]{0,160}pointer-events:\s*auto/);
  ...
  assert.match(appSource, /const timelineProgress = `\$\{clamp\(timeline\.progress, 0, 1\) \* 100\}%`/);
  assert.match(appSource, /progress=\{metadata\.activeAnimation === name \? timelineProgress : undefined\}/);
  assert.match(appSource, /<i className="timeline-progress" style=\{\{ width: progress \}\} aria-hidden="true" \/>/);
  assert.match(styleSource, /\.timeline-progress\s*\{[\s\S]{0,120}position:\s*absolute;[\s\S]{0,160}background:\s*rgb\(242 201 76 \/ \.32\);[\s\S]{0,120}transition:\s*width 100ms linear/);
  assert.doesNotMatch(styleSource, /var\(--timeline-progress\)/);
  ...
  assert.match(playerSource, /function getAnimationDuration\([\s\S]{0,800}duration \/ fps/);
  assert.match(playerSource, /this\.animationDuration = getAnimationDuration\(definition, this\.animation\)/);
  assert.match(playerSource, /this\.animationLoopValue = getAnimationLoopValue\(definition, this\.animation\)/);
  assert.match(playerSource, /this\.publishProgress\(\);[\s\S]{0,100}this\.callbacks\.onPlayback\(true/);
  assert.match(playerSource, /this\.animationElapsed \+= delta/);
  assert.match(playerSource, /this\.publishProgress\(true\);[\s\S]{0,100}this\.callbacks\.onPlayback\(false/);
  assert.doesNotMatch(playerSource, /isLoopingIdle|activeAnimationName\.trim\(\)\.toLowerCase\(\) === "idle"/);
